...
- **`chat.ts`** — Main text message handler; routes to streaming prompt pipeline. Supports reply-to-message context (including voice/audio replies via STT transcription) and edited message re-prompting. Edited messages do NOT consume pending typed answers (only new messages do). Checks for pending command input before treating text as a prompt
...
- **`session.ts`** — `/new`, `/sessions`, `/switch`, `/delete`, `/current`, `/rename`. `/switch` and `/delete` without arguments show session picker keyboards. `/delete` has confirmation flow. `/rename` without arguments uses `promptForInput()`
- **`monitor.ts`** — `/todo`, `/diff`, `/fork`
- **`files.ts`** — `/ls`, `/read`, `/find`, `/search`, `/symbols`, `/status`. Commands without arguments use `promptForInput()` for interactive input
...
- **`system-prompt.ts`** — Loads custom system prompt from `{dataDir}/SKILL.md` (or `systemPromptFile` config), watches for hot-reload. No cwd fallback (removed in v2.5.8). Conditionally appends MCP tool instructions (Browser, Fetch, Memory, Filesystem, GitHub, Context7, Relay) based on config flags. Appends fresh timestamp in configured timezone to every prompt via `getSystemPrompt()` — uses `getTimezoneAbbr()` from `cron.ts` for clean timezone abbreviations (e.g. `IST` instead of `GMT+5:30`). Also exports `writeSystemPromptFile()` which assembles the full prompt (without timestamp) and writes it to `{dataDir}/RELAY.md` for delivery via OpenCode's `instructions` config. Hot-reload rewrites `RELAY.md` automatically when `SKILL.md` changes
- **`input.ts`** — Interactive command input utility. `promptForInput(ctx, message, handler)` sends a `force_reply` message and registers a handler for the next text reply. Used by commands that can be invoked without arguments (`/read`, `/find`, `/search`, `/symbols`, `/shell`, `/rename`, `/mcp remove`, `/mcp connect`). Handlers expire after 5 minutes. `consumePendingInput()` is checked in `chat.ts` before treating text as a prompt
...
- **`src/session.ts`** — Tracks active session ID, selected model, selected agent, and selected STT provider, persisted to `~/.relay/session.json`. Uses a mutex to prevent race conditions on concurrent messages. Exports `withPromptQueue()` for serial prompt execution (prevents SSE stream interleaving). `getOrCreateSession()` validates cached session IDs against OpenCode (handles stale IDs after server restarts).
...
- **`src/mcp/relay-server.ts`** — Standalone MCP stdio server spawned by OpenCode. Exposes 8 tools: `cron_list`, `cron_add`, `cron_update`, `cron_remove`, `cron_toggle`, `cron_run`, `notify`, `health` (without `relay_` prefix — OpenCode auto-prepends the server name `relay_` to produce `relay_cron_list`, etc.). Schedule types: `interval`, `daily`, `weekly`, `once`. Tool descriptions reference user's configured timezone. Cron list output shows timezone header and all times in user's timezone. Uses newline-delimited JSON-RPC over stdio. Communicates with `relay-api.ts` via localhost HTTP (no auth).
...
- **Reply context**: When users reply to a bot message, the quoted text is prepended to the prompt as `[Replying to: "..."]`. Supports replying to voice/audio messages — transcribes the audio via STT and prepends as `[Replying to voice message: "..."]` or `[Replying to audio: "..."]`. Media handlers also support reply context for photos, documents, voice, and audio.
...
- **Tool display**: Tool use events show human-readable labels with emoji indicators: `🔧 Label...` (running), `✅ Label` (completed), `❌ Label` (error). Labels prefer `part.state.title` (e.g., "Read file src/index.ts") when available, falling back to `formatToolName()` which strips known MCP server prefixes (`relay_`, `playwright_`, `fetch_`, `memory_`, `filesystem_`, `github_`, `context7_`) and title-cases the remainder.
...
- **Cron schedule types**: `interval` (every N minutes), `daily` (once per day), `weekly` (specific days), `once` (fires once at specified time, then auto-disables with `enabled=false`, preserving execution history). Toggle re-enables for another run. All cron times use the user's configured timezone. Each job runs in an isolated session (created fresh per run, deleted after) so cron output doesn't pollute the user's conversation. During execution, an animated dots indicator ("Running.", "Running..", "Running...") cycles in the header message. A pre-flight `ensureServerAlive()` check verifies the AI server before executing.
...
- **Relay MCP**: Relay starts an internal HTTP API on `127.0.0.1:relayMcpPort` (default 39149) and registers a `relay` MCP server in OpenCode's config. The MCP server is a separate stdio process (`src/mcp/relay-server.ts`) spawned by OpenCode, which calls the internal API on localhost (no auth — localhost-only binding). Uses `@modelcontextprotocol/sdk` with newline-delimited JSON-RPC (not Content-Length framed). Tool registration names omit the `relay_` prefix (e.g., `cron_list`) because OpenCode auto-prepends the server name — final tool names seen by the AI are `relay_cron_list`, etc.
- **Interactive command input**: Commands that accept arguments (`/read`, `/find`, `/search`, `/symbols`, `/shell`, `/rename`, `/mcp remove`, `/mcp connect`) can be invoked without arguments — they prompt the user for input via `force_reply` using `promptForInput()` from `src/utils/input.ts`. The next text message is consumed by the handler instead of being sent as a prompt. Handlers expire after 5 minutes.
- **Session picker keyboards**: `/switch` and `/delete` without arguments show interactive session picker keyboards. `/delete` includes a confirmation flow (Yes/No keyboard). `/mcp remove` also shows a confirmation keyboard.
