...
- `claudeModel` — Model name or ID (default: `sonnet`)
- `claudePermissionMode` — How Claude handles file edits (default: `acceptEdits`)
- `claudeCwd` — Working directory for Claude (default: current directory)

The `ANTHROPIC_API_KEY` environment variable must be set in the coding agent's environment.

### OpenAI Codex

- `codexModel` — Model name or ID (default: `o3`)
- `codexCwd` — Working directory for Codex (default: current directory)

The `CODEX_API_KEY` or `OPENAI_API_KEY` environment variable must be set in the coding agent's environment.
...
- `sttProvider` — STT provider: `groq`, `openai`, `assemblyai`, or `auto` (default: `auto`)
