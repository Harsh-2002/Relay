...
- **Tool indicators**: "Reading file...", "Running command...", etc. appear during processing
...
Four STT providers are supported: **Groq** (fastest, free tier), **Sarvam AI** (Indian language support), **AssemblyAI**, and **OpenAI**. When `sttProvider` is `auto` (default), the cheapest configured provider is auto-selected. If it fails, other configured providers are tried automatically.
...
Switch between AI models at runtime. `/models` displays an interactive inline keyboard with capability badges: `[reasoning]`, `[vision]`, `[free]`. The active model is shown with a `✓` prefix. Tap a button to select a model directly.
...
Jobs are persisted to `cron.json` and survive restarts. The scheduler checks for due jobs every 30 seconds. If the bot restarts, missed jobs are skipped (no avalanche of past runs).
